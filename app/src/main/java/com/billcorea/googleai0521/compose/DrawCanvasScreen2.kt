...
@Composable
@Destination
fun DrawCanvasScreen2(
    navigator: DestinationsNavigator,
    bakingViewModel: BakingViewModel,
    doUpdateImageArray: (idx: Int, leftRight:String, bitmap: Bitmap, b: Boolean) -> Unit,
) {
    ...
    // 기본 공유 시트와 공유합니다.
    fun shareBitmapFromComposable(idx: Int, leftRight: String) {
        if (writeStorageAccessState.allPermissionsGranted) {
            coroutineScope.launch(Dispatchers.IO) {
                val bitmap = createBitmapFromPicture(picture)
                doUpdateImageArray(idx, leftRight, bitmap, true)
                Log.e("", "$idx ${bakingViewModel.bitmaps.size}")
    ...
    LazyColumn(
        ...
        item{
            Row (
                ...
                IconButton(onClick = {
                    shareBitmapFromComposable(bakingViewModel.idx.value, bakingViewModel.leftRight.value)
                    navigator.popBackStack()
                }) {
